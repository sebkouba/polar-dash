...
Polar Dash is a fast personal prototype for turning a Polar H10 chest strap into a single macOS app for live breathing and heart-rate monitoring.
...
- `macos/BreathingBar`: the main Swift app. It owns BLE collection, live breathing/HR/HRV estimation, bundled calibration loading, derived graph-history storage, the dashboard window, and the menu bar UI.
- `src/polar_dash/breathing.py`: reference Python implementation of the ACC, ECG, RR, and fusion-based breathing estimators used for experimentation and comparison work.
- `src/polar_dash/evaluate.py`: scoring utilities for estimates versus saved labels.
- `src/polar_dash/cockpit.py`, `src/polar_dash/dashboard.py`, `src/polar_dash/labeler_v2.py`: older Python tooling kept around for calibration, labeling, and offline inspection when needed.
...
If you specifically want the older browser-based view:
...
The older Python cockpit shows the shortcuts in the sidebar, and the label keys are active once you start a label session:
...
- The Swift app is macOS-only.
- The remaining Python tools are intentionally utilitarian; they still exist for experiments and recalibration, not as the primary product.
